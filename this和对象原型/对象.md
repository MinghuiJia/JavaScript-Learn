 ...
 * @LastEditTime: 2022-12-29 15:16:10
...
11. 存在性
    ...
    - `in`检查的是某个属性名是否存在，而**不是某个值**，因此`4 in [2, 4, 6]`的结果是false，数组的索引只有0，1，2。查找数组中是否存在某个元素的方法：`arr.indexOf()`、`arr.find((value, index, arr) => {})`、`arr.some((value, index, arr) => {})`、`arr.includes()`
12. 枚举
    ```javascript
    var myObject = { };
    Object.defineProperty(
