 ...
 * @LastEditTime: 2023-02-11 18:06:31
...
4.  数字
    ...
    - 32位有符号整数。虽然整数最大能够达到53位，但是有些数字操作（如数位操作）只适用于32位数字，所以这些操作中数字的安全范围就要小很多，变成从`Math.pow(-2,31)`（-2147483648，约－21亿）到`Math.pow(2,31) - 1`（2147483647，约21 亿）。**因为是有符号的32位，第一位要用于设置正负，所以才是2的31次方；-1的原因是位数是从0开始的，31位都是1等价于32位是1其余位是0的十进制数再减一**
        - `a | 0`可以将变量a中的数值转换为32位有符号整数，因为数位运算符`|`只适用于32位整数。因此与0进行操作即可截取a中的32位数位（截取方式是：超出范围的高字节部分会被截掉，最低位在最右侧）
...
5.  特殊的数值
    ...
    - 特殊等式
        - ES6中新加入了一个工具方法`Object.is(..)`来判断两个值是否绝对相等，可以解决0与-0；NaN的判断
            ```javascript
            var a = 2 / "foo";
            var b = -3 * 0;
...
7.  宽松相等（`==`）和严格相等（`===`）
    ...
    - 抽象相等
        ...
        - null和undefined之间的相等比较
            ...
            - null与undefined常用的场景
                ...
                ```
        - 对象和非对象之间的相等比较
            - 关于对象（对象/函数/数组）和标量基本类型（字符串/数字/布尔值）之间的相等比较，规范规定需要将对象变量进行`ToPrimitive`抽象类型转换，而这里只提到了字符串和数字，没有布尔值。原因是布尔值会先被强制类型转换为数字
                ```javascript
                var a = 42;
                var b = [ 42 ];
                a == b; // true
                ```
                - `[ 42 ]`首先调用`ToPromitive`抽象操作返回"42"，变成`"42" == 42`，然后又变成`42 == 42`，最后二者相等
            - `==`中的`ToPromitive`强制类型转换也“拆封”对象，返回其中的基本数据类型值
                ```javascript
                var a = "abc";
                ...
                a == b; // true
                ```
                - `a == b`结果为true，因为b通过`ToPromitive`进行强制类型转换（也称为“拆封”），并返回标量基本类型值"abc"，与a相等
            - 但是在null、undefined、NaN中会出现例外
                ```javascript
                var a = null;
                ...
                e == f; // false
                ```
                - 因为没有对应的封装对象，所以null和undefined不能够被封装，均返回一个常规对象
                - NaN能够被封装为数字封装对象，但拆封之后`NaN == NaN`返回false，因为NaN不等于NaN
    - 比较少见的情况
        - 返回其他数字
            ```javascript
            ...
            new Number( 2 ) == 3; // true
            ```
            - `Number(2)`涉及`ToPrimitive`强制类型转换，会调用`Number.prototype.valueOf()`方法
            ...
            ```
            - `a.valueOf()`每次调用都产生副作用，比如第一次返回2，第二次返回3，是的两次判断的结果都是true
        - 假值的相等比较
            ```javascript
            "0" == null; // false
            "0" == undefined; // false
            ...
            0 == {}; // false
            ```
            - 以上24种情况中有17种比较好理解。比如我们都知道""和NaN不相等（因为null只与undefined相等，其余都不相等），"0"和0相等。注意`{}`对象进行`ToString`隐式转换时，会返回`[object Object]`字符串
            - 然而有7种我们注释了“晕！”，因为它们属于假阳（false positive）的情况，""和0明显是两个不同的值，它们之间的强制类型转换很容易搞错
        - 极端情况
            ```javascript
            [] == ![] // true
            ```
            - 看起来这似乎是真值和假值的相等比较，结果不应该是true，因为一个值不可能同时既是真值也是假值
            - 根据`ToBoolean`规则，它会进行布尔值的显式强制类型转换（同时反转奇偶校验位）。所以`[] == ![]`变成了`[] == false`。然后`[] == false`再转换为`[] == 0`，返回true

            ```javascript
            2 == [2]; // true
            "" == [null]; // true
            ```
            - 介绍`ToNumber`时我们讲过，`==`右边的值`[2]`和`[null]`会进行`ToPrimitive`强制类型转换，以便能够和左边的基本类型值（2和""）进行比较
            - 第一行中的`[2]`会转换为"2"，然后通过`ToNumber`转换为2。第二行中的`[null]`会直接转换为""

            ```javascript
            0 == "\n"; // true
            ```
            - ""、"\n"（或者" "等其他空格组合）等空字符串被`ToNumber`强制类型转换为0
    - 常规的安全的相等
        ```javascript
        42 == "43"; // false
        "foo" == 42; // false
    ...
    - 完整性检查
        - 之前列举的24中比较情况下，有7种强制类型转换的坑
            ...
            0 == []; // true -- 晕！
            ```
            - 其中有4种情况涉及`== false`，之前我们说过应该避免，应该不难掌握。注意：""会被`ToNumber`强制转换成0，而[]也会被`ToString`转换成""再转换成0。`== false`的比较会先将boolean类型转换成number
        - 剩下的3种
            ```javascript
            "" == 0; // true -- 晕！
            "" == []; // true -- 晕！
        ...
        - 一般情况下我们会用`== ""`或者`== 0`
            ...
            ```
            - 如果不小心碰到`doSomething(0)`和`doSomething([])`这样的情况，就会出现`0 == ""`与`[] == ""`导致true的结果
    - 安全运用隐式强制类型转换
        - 如果两边的值中有true或者false，千万不要使用`==`
        - 如果两边的值中有[]、""或者0，尽量不要使用`==`
        - 这时最好用`===`来避免不经意的强制类型转换，避开强制类型转换的坑（参照上述两个原则几乎可以避开所有的强制类型转换坑）
        - 有一种情况下强制类型转换是绝对安全的，那就是`typeof`操作。`typeof`总是返回七个字符串之一（string、number、boolean、object、function、undefined、symbol），其中没有空字符串。`typeof x == "function"`是100%安全的
8.  抽象关系比较
    - `a < b`中涉及的隐式强制类型转换不太引人注意，规范规定了比较双方都是字符串和和其他情况
    ...
    - 如果比较双方都是字符串，则按字母顺序来进行比较
        ...
        a < b; // false
        ```
        - a和b并没有被转换为数字，因为`ToPrimitive`返回的是字符串，所以这里比较的是"42"和"043" 两个字符串，它们分别以"4"和"0"开头。因为"0"在字母顺序上小于"4"，所以最后结果为false
        ...
        a < b; // ??
        ```
        - 结果还是false，因为a是`[object Object]`，b也是`[object Object]`
        - 注意：对象类型转换为字符串，如果没有定义`toString()`方法，返回的结果都是对象内的`[[class]]`属性
    - 奇怪的例子
        ```javascript
        var a = { b: 42 };
        var b = { b: 43 };
        ...
        a >= b; // true
        ```
        - 为什么`a == b`的结果不是true，因为这是涉及对象的`==`比较，两个对象引用进行比较时，如果引用的是同一个值才会返回true
        - `a <= b`和`a >= b`的结果会是true，是因为`a <= b`被处理为`b < a`，然后将结果反转——（`!(a > b)`）
    - 相等比较有严格相等，关系比较却没有“严格关系比较”，避免的办法只能确保a和b为相同的类型
