 ...
 * @LastEditTime: 2023-01-10 22:13:35
...
6.  隐式强制类型转换
    ...
    -  字符串和数字之间的隐式强制类型转换
        ...
        c + d; // 42
        ``` 
        - +运算符通过重载，既能用于数字加法，也能用于字符串拼接。通常当某一个或者两个操作数都是字符串，所以+执行的是字符串拼接操作，但实际情况要复杂的多
        ```javascript
        var a = [1,2];
        var b = [3,4];
        a + b; // "1,23,4"
...
7.  宽松相等（`==`）和严格相等（`===`）
    ...
    - 抽象相等
        ...
        - null和undefined之间的相等比较
            ...
            - null与undefined常用的场景
                ...
                ```
                - 条件判断`a == null`仅在`doSomething()`返回非null和undefined时才成立，除此之外其他值都不成立，包括0、false和""这样的假值
    ...
    - 比较少见的情况
        ...
        - 假值的相等比较
            ...
            0 == {}; // false
            ```
            - 以上24种情况中有17种比较好理解。比如我们都知道""和NaN不相等（因为null只与undefined相等，其余都不相等），"0"和0相等
    ...
    - 完整性检查
        - 之前列举的24中比较情况下，有7种强制类型转换的坑
            ...
            0 == []; // true -- 晕！
            ```
            - 其中有4种情况涉及`== false`，之前我们说过应该避免，应该不难掌握。注意：""会被`ToNumber`强制转换成0，而[]也会被`ToString`转换成""再转换成0
        - 剩下的3种
            ```javascript
            "" == 0; // true -- 晕！
            "" == []; // true -- 晕！
